...
execute unless entity @s[nbt={HandItems:[{},{tag:{Aging:5,Fertility:4,Leather:4,Beef:5}}]}] run clear @p[tag=genetics.caster,gamemode=!creative] minecraft:firework_star{display:{Name:'{"text":"Potent Mutation Catalyst","color":"light_purple","italic":false}'}} 1
execute unless entity @s[nbt={HandItems:[{},{tag:{Aging:5,Fertility:4,Leather:4,Beef:5}}]}] run function genetics:strong/cow/branch
